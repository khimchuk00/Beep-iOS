...
//  BeepIos
//
//  Created by Valentyn Khimchuk on 19.11.2021.
//

import UIKit

protocol EditAssetViewControllerDelegate: AnyObject {
    func updateCell()
}

class EditAssetViewController: OffsetableViewController {
    // MARK: - Outlets
    ...
    private var viewModel: ContModel!
    private var mainTableRow: MainTableRow!
    
    weak var delegate: EditAssetViewControllerDelegate?
    ...
    func configure(with model: ContModel, delegate: EditAssetViewControllerDelegate?) {
    ...
    @IBAction func saveButtonDidTap() {
        ...
        NetworkManager.shared.updateContact(id: viewModel.id, contactTypeId: viewModel.contactTypeId, title: title, value: value, isActive: viewModel.isActive) { [self] data in
    ...
    private func localize() {
        ...
        linkTextField.text = viewModel.value
    ...
    private func configureImageView() {
        ...
        if let url = URL(string: "https://server.beep.in.ua/uploads/contact_types/\(viewModel.contactTypeIcon)") {
...
extension EditAssetViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
